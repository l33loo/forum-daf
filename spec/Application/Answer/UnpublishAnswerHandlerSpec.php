...
/**
 * UnpublishAnswerHandlerSpec specs
 *
 * @package spec\App\Application\Answer
 */
class UnpublishAnswerHandlerSpec extends ObjectBehavior
{
    private $answerId;
    ...
    function let(
        ...
        $answer->unpublish()->willReturn($answer);
    ...
    {
        $this->shouldHaveType(UnpublishAnswerHandler::class);
    }
    ...
    function it_handles_publish_answer_command(
        ...
        $command = new UnpublishAnswerCommand($this->answerId);
        $this->handle($command)->shouldBe($answer);
        $answer->unpublish()->shouldHaveBeenCalled();
